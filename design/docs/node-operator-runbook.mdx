...
This is a **living document**. Hashi launches on Testnet first. This document fills Mainnet parameters into the [Networks](#networks) table below as the team finalizes them. Wherever the runbook shows a network-specific value, use the column for your **target network** (the worked examples use Testnet).
...
Hashi connects to Sui over the **gRPC v2 API (gRPC over HTTP/2)**, **not** JSON-RPC. The fullnode must expose the `sui.rpc.v2` services, including the **streaming `SubscriptionService`** (`subscribe_checkpoints`) the node uses to follow the chain. A fullnode serving only JSON-RPC does not work.
...
| **TLS private key** | Ed25519 | Reuse Sui validator TLS key | Peer-to-peer authentication through a self-signed cert. Public key registered onchain. |
...
Reuse the Ed25519 TLS key from your Sui validator infrastructure. This key authenticates your Hashi node to other committee members; the corresponding public key is registered onchain during registration. Set `tls-private-key` to a PKCS#8 PEM/DER file path or an inline PEM string.
...
Your **Sui validator account key must sign and broadcast the initial registration transaction.** Onchain, `validator::register` sets the member's `validator_address = ctx.sender()` and asserts the sender is in the Sui active validator set, so no operator key can substitute for this first transaction.
...
- The TLS public key is registered onchain, so peers verify certificates against the onchain registry, with no certificate authority needed.
...
> **Coming soon:** A metrics **push** service for centralized collection. Details are added here when available.
