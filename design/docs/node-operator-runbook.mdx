...
A single run builds the image once and pushes both `us-central1-docker.pkg.dev/cryptic-bolt-398315/hashi/hashi:<sha>` and the public `mysten/hashi:<sha>`. Artifact Registry rejects anonymous pulls, so announcements should point operators at Docker Hub. Confirm the tag is live before announcing:
...
Proceed to the next node only after all three checks pass. Do not gate on `kubectl wait --for=condition=Ready pod -l hashi-node=mysten<N>` — the label also matches the still-terminating old pod. If the fleet is unhealthy to begin with (for example mid reconfiguration stall, §9.3), `rollout status` never sees Ready; gate on the pod reaching `Running` with no restarts plus the guardian bootstrap log line instead. A node that fails to come back healthy means stop and investigate before touching the next — every additional node down eats into the committee's 2/3 signing margin (§9).
