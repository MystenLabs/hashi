...
- Any committee member can create a proposal. Quorum is stake-weighted, and the required threshold depends on the proposal type: most actions need **2/3 of committee weight**, but package-version changes (Upgrade, EnableVersion, DisableVersion) require a **unanimous 100%**. See the threshold column in [7.2](#72-proposal-types).
...
| **Upgrade** | 100% (unanimous) | Publish a new package version | `--package-path` or `--digest` |
| **EnableVersion** | 100% (unanimous) | Re-enable a disabled package version | `version` number |
| **DisableVersion** | 100% (unanimous) | Disable a package version | `version` number (cannot disable active version) |
