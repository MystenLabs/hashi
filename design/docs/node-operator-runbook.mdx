...
This is a **living document**. Hashi launches on testnet first; mainnet parameters are filled into the [Networks](#networks) table below as they are finalized. Wherever the runbook shows a network-specific value, use the column for your **target network** — the worked examples use Testnet.
...
> A self-hosted Sui fullnode is reached at its RPC port (default `http://127.0.0.1:9000`) on any network. Note that Hashi's built-in default for `bitcoin-rpc` is `http://localhost:8332` (the mainnet port), so on Signet you must set the port explicitly.
...
Set `sui-chain-id` and `sui-rpc` to match your target network — see the [Networks](#networks) table.
