---
title: Introduction
description: High-level overview of Hashi, the Sui native Bitcoin orchestrator that secures and manages BTC for use on Sui through threshold cryptography.
keywords: [ Hashi, Bitcoin, Sui, BTC, hBTC, threshold cryptography, MPC, bridge ]
slug: /
sidebar_label: Introduction
sidebar_position: 1
---

This is a high-level overview and design doc for `hashi`, the Sui native
Bitcoin orchestrator. The document is a living reference, updated as new
decisions and features land, with the goal of being a canonical description
of how Hashi is designed and operates.

At a high level, `hashi` is a protocol for securing and managing BTC for use
on the Sui blockchain using threshold cryptography.

The first feature that Hashi supports is the ability to deposit and withdraw
BTC to a managed pool, with ownership represented as a fungible `Coin<BTC>`
on Sui.

:::danger
**There is no `$HASHI` token.** The only coin the Hashi protocol creates is
`hBTC`, the Bitcoin-backed asset minted on deposit. Hashi has no governance,
utility, or airdrop token, and governance is carried out by committee members
voting onchain with their validator stake weight, not with a token. Treat any
`$HASHI` token offering, presale, or airdrop as an imitation.
:::
