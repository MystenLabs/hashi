...
sequenceDiagram
    ...
    Operator->>Operator: If no serving committee exists, build GenesisState from onchain state
    ...
    KPs->>KPs: Verify PCRs and independently derive both state hashes
    ...
    alt fewer than threshold shares collected
        ...
        opt first deploy
            Guardian->>S3: log(genesis committee plus exact KP-signed approvals)
        end
...
On first deploy, the operator pins the current onchain committee as an optional
`GenesisState` during `OperatorInit`. Each KP independently derives its hash and
includes it in the normal signed PI submission. Once PI reaches threshold, the
enclave writes `genesis/record.json` with the committee and the exact signed
submissions that authorized it. Later deploys use `None` and do not write genesis.
