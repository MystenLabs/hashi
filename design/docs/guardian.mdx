...
On first deploy, the operator and each KP pass `--do-genesis`. This is purely an
...
the enclave writes `genesis/record.json` with the committee and the exact signed
submissions that authorized it. Later deploys use `None` and do not pass the flag.
