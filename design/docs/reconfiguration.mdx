...
[abort reconfig](#abort-reconfig) can resolve the pending state. Nothing
pending at all is reported as `EReconfigAlreadyCompleted` when the
submission's own target already activated (another node's `end_reconfig` won
the race, which the node treats as success) and as `ENotReconfiguring`
otherwise (the target was aborted). Nodes key their retry-or-give-up decision
on which of these fires.
...
3. The launch switch, exactly as for `start_reconfig`.
