...
target fails verification against whatever is pending. Nothing pending at
all is reported as `EReconfigAlreadyCompleted` when the submission's own
target already activated (another node's `end_reconfig` won the race, which
the node treats as success) and as `ENotReconfiguring` otherwise (the target
was aborted). Handoffs are stored by source epoch, and a replacement can form
from the same source epoch after an abort, so for a handoff "own target" is
established by verifying the certificate against the stored transition out
of its signing epoch. Nodes key their retry-or-give-up decision on which of
these fires.
