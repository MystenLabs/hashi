...
```mermaid
graph LR
    A[Start Reconfig] --> B[DKG or Key Rotation] --> C[End Reconfig] --> D[Guardian Committee Update]
```
...
```mermaid
graph LR
    A[Start Reconfig]:::active --> B[DKG or Key Rotation] --> C[End Reconfig] --> D[Guardian Committee Update]
...
```mermaid
graph LR
    A[Start Reconfig] --> B[DKG or Key Rotation]:::active --> C[End Reconfig] --> D[Guardian Committee Update]
    classDef active fill:#298DFF,stroke:#1759C4,color:#FFFFFF
```
...
```mermaid
graph LR
    A[Start Reconfig] --> B[DKG or Key Rotation] --> C[End Reconfig]:::active --> D[Guardian Committee Update]
    classDef active fill:#298DFF,stroke:#1759C4,color:#FFFFFF
```
...
### Guardian committee update

```mermaid
graph LR
    A[Start Reconfig] --> B[DKG or Key Rotation] --> C[End Reconfig] --> D[Guardian Committee Update]:::active
    classDef active fill:#298DFF,stroke:#1759C4,color:#FFFFFF
```

After the onchain epoch advances, the guardian service must also update its
view of the active committee. A committee member calls the guardian's
`UpdateCommittee` RPC, passing a `SignedCommitteeTransition` that contains
the new committee information along with a threshold signature from the
outgoing committee. The guardian performs the following steps:

1. **Verifies the outgoing committee's threshold signature** to authenticate
   the transition request.
2. **Swaps its in-memory committee** to the new one so that subsequent
   requests are validated against the updated membership.
3. **Logs the transition record to S3** for auditability.

This step ensures that the guardian recognizes the new committee and accepts
requests from its members going forward. Until the `UpdateCommittee` RPC
completes successfully, the guardian continues to operate with the previous
committee view.
