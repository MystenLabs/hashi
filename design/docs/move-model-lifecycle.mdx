...
| 4    | Committee members screen and sign request        | Each member screens with TRM Labs; BLS signatures aggregated → `CommitteeSignature`   |
| 5    | Leader calls `approve_deposit()` with cert       | `verify_certificate()` → request stores `approval_cert` and `approval_timestamp_ms`   |
| 6    | Time-delay window elapses                        | `bitcoin_deposit_time_delay_ms` (allows committee rotation if approval is fraudulent) |
| 7    | Anyone calls `confirm_deposit()`                 | Re-`verify_certificate()` → `CertifiedMessage<DepositRequest>`                        |
| 8    | Certified request processed                      | `DepositRequest` → `Utxo` in `UtxoPool`                                               |
| 9    | Treasury mints tokens                            | `Utxo.amount` → `Balance<BTC>` to user                                                |
