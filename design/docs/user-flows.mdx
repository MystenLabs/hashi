...
Hashi's Testnet deployment uses **Bitcoin Signet**, so a test deposit needs Signet BTC. Signet coins have no value and cannot be bought or mined in any practical sense; they are handed out by faucets.
...
:::info
Your own Signet node cannot create spendable coins for you. Signet block production is centrally signed, so a faucet is the normal way to obtain test BTC.
:::

Once you hold Signet BTC, the deposit loop is the one described above, driven through the CLI. Global options attach to the first-level subcommand, so `-c` goes after `deposit` (see the [CLI reference](node-operator-runbook.mdx#61-global-options)):
...
The deposit must meet Bitcoin's dust minimum, and Hashi waits for a confirmation threshold before minting, so expect to wait for several Signet blocks between steps 3 and 5. See [Deposit](deposit.mdx) for the request, approve, confirm, and mint phases in detail.
