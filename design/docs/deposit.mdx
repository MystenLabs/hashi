---
title: Deposit
description: How Hashi turns BTC sent to a deposit address into hBTC on Sui through the request, confirm, and mint phases.
keywords: [ deposit, hBTC, mint, UTXO, confirmation, Bitcoin, Sui ]
---
...
that considers the address sanctioned does not vote to accept the deposit.
...
deposit should be confirmed. If a quorum of validators cannot agree that a
deposit should be confirmed, the request is either retried later or ignored
if invalid.
...
After a quorum of validators agrees that a deposit should be confirmed, one
validator submits the certificate onchain by calling `hashi::deposit::confirm_deposit`:
