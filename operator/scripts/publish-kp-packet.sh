...
USAGE="Usage: $0 <name> <phase> <bundle-dir> [--attestation mock]"
NAME=""
PHASE=""
BUNDLE_DIR=""
ATTESTATION=real
expect_attestation=false
for argument in "$@"; do
  if [[ "$expect_attestation" == true ]]; then
  ...
  case "$argument" in
    ...
    -h | --help)
      printf '%s\n' "$USAGE" \
        ...
        "roster certificate and its attestation files." \
        "Pass --attestation mock only for a guardian built with non-enclave-dev; key provisioners then" \
        "build their tools the same way. See operator/README.md."
      exit 0
      ;;
    *)
      if [[ -z "$NAME" ]]; then
...
case "$ATTESTATION" in
  mock | real) ;;
  *) die "Attestation must be mock or real, not $ATTESTATION. $USAGE" ;;
esac
[[ "$expect_attestation" == false ]] || die "--attestation needs a value. $USAGE"
...
printf 'Phase:           %s\nAttestation:     %s\nCertificates:    %s\nSharing:         %s-of-%s\nGuardian bucket: s3://%s\n' \
  "$PHASE" "$ATTESTATION" "$(grep -c . < "$WORK_DIR/certs" || true)" "$THRESHOLD" "$NUM_SHARES" "$GUARDIAN_BUCKET"
if [[ "$ATTESTATION" == mock ]]; then
  printf '\nWARNING: this packet is for a guardian with mock attestation, which is never production.\n' >&2
fi
...
# Load the packet's own configuration the way every production command does, from the packet
# directory, so its relative certificate paths resolve exactly as a key provisioner's will.
# A real packet is checked by a build that trusts only Yubico; a mock one by the build its key
# provisioners will use, which also trusts a software attestation device.
...
if ! (cd "$PACKET_DIR" && cargo run --release --locked --quiet \
  --manifest-path "$REPO_ROOT/Cargo.toml" -p hashi-guardian-init \
  ${BUILD_FEATURES[@]+"${BUILD_FEATURES[@]}"} -- \
  tools check-config --config guardian-init.yaml < /dev/null); then
  die "The packet holds a configuration the guardian tools reject, so nothing was published."
fi
