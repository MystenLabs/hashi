...
#                   the quiet window.
#
# `--do-genesis` rides `full` only: it bootstraps the FIRST serving committee,
# and a standby is provisioned into a chain that already has one.
#
# The steps:
...
# Genesis belongs to the first bring-up only; arming a standby must not
# re-bootstrap a committee that already exists.
GENESIS_ARGS=()
if [ "${MODE}" = "full" ]; then
  GENESIS_ARGS=(--do-genesis)
fi
