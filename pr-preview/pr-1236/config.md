...
| **Type** | `u64` |
| **Default** | `1` |
| **Valid range** | `1` and up |

This key sets which version of the MPC signing logic a committee uses,
including the weight reduction. Nodes won't take part in a committee pinned to
a version their release doesn't support, so only raise it after the fleet has
upgraded.
