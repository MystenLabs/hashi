...
Bitcoin transaction and Sui recipient with its own TRM Labs account (see
[Handling Sanctioned Addresses](sanctions.mdx)). A member that rejects the
deposit does not vote to approve it.
...
Each member's signature also covers the UTXO's spend data: its output script,
its 2-of-2 leaf and control block, the key path the MPC signs under, and the
sighash type. Each member computes this from its own keys and checks the
Bitcoin output against it, so a certificate forms only on spend data a quorum
computed. Withdrawals later sign from this stored data instead of deriving it
again.
...
The function verifies the committee certificate over the request, its UTXO and
`spend` against the current committee, and records the certificate, `spend`
and the current clock timestamp on the request. The request remains in the
deposit queue, and no `hBTC` is minted yet.
