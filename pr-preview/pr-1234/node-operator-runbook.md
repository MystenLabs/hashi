...
| [14. Releases that change signing](#14-releases-that-change-signing) | The release rule for signing changes, and how a new signing version ships |
...
| `hashi_spend_check_refusals_total` | counter | Operations this node refused because a UTXO's spend data was missing or wrong, or a txid, the sighash digest or the presignature pairing did not match what it computed (by `site`, `check`). Healthy: flat. |
| `hashi_utxo_selection_skipped_records` | gauge | UTXOs skipped because their spend record failed a check (by `reason`), as of the latest coin selection this node ran as leader. Healthy: `0`. |
| `hashi_mpc_signing_version_supported` | gauge | 1 for each signing version this binary supports (label `version`, section 14). |
| `hashi_mpc_signing_version_refused_total` | counter | Refusals of an epoch this node cannot compute, by `reason`: `unsupported` (its release lacks the epoch's signing version, given in `version`), `missing` or `malformed` (the committee's config has no valid `mpc_signing_version`), or `legacy` (the committee still carries a retired key such as `mpc_threshold_in_basis_points`). Healthy: flat. |
| `hashi_mpc_reduction_version_info` | gauge | 1 for each signing version this binary supports, labelled `version` and `source_digest`, a digest of that version's weight-reduction source. Every release that supports a version must report the same digest for it. |
| `hashi_mpc_reduction_info` | gauge | 1 for each epoch the node reads, labelled `epoch`, `version` and `digest`, a digest of that epoch's weight reduction. Every node should show the same digest for an epoch. |
...
| `increase(hashi_spend_check_refusals_total[1h]) > 0 or (hashi_spend_check_refusals_total unless hashi_spend_check_refusals_total offset 1h) or (hashi_spend_check_refusals_total and on(job, instance) uptime < 3600)` | Warning | A spend check refused work on this node; the `site` and `check` labels say where and which. Ask in the operator channel whether peers refuse too: if only this node does, its release differs from the fleet's (section 14); if every other node refuses at `site="commit"`, the leader's proposal is at fault. |
| `increase(hashi_mpc_signing_version_refused_total[10m]) > 0 or (hashi_mpc_signing_version_refused_total unless hashi_mpc_signing_version_refused_total offset 10m) or (hashi_mpc_signing_version_refused_total and on(job, instance) uptime < 600)` | Critical | This node sits an epoch out. For `reason="unsupported"`, upgrade to a release that supports `version` (section 14). The other reasons mean the committee's config does not fit this release: `missing` or `malformed` (no valid `mpc_signing_version`, for example on a package published before the key) or `legacy` (a retired key such as `mpc_threshold_in_basis_points`). Check that the node runs the network's release; if it does, escalate. |
...
- **Apply releases that change signing only as §14 describes.** Their new behavior starts when governance raises the signing version, at the next committee formation, never node by node.
...
Proceed to the next node only after all three checks pass. If a node does not come back healthy, stop and investigate before touching the next — every additional node down eats into the committee's 2/3 signing margin (§9). Never roll a release that changes signing (§14) this way: testnet's committees have no signing version, so old and new nodes would sign the same epoch differently. Hold such a release until testnet has its own plan.
...
## 14. Releases that change signing

This applies to networks whose committees pin a signing version, such as mainnet. Testnet does not (§13.3).

If old and new nodes sign the same epoch differently, the committee's Bitcoin key can leak. So a release never changes how an existing epoch is signed: a signing change ships as a new signing version, which governance switches for every node at once at a committee formation. The golden signing tests must pass unchanged, and any release that touches signing code or bumps fastcrypto is reviewed as a signing change.

To ship one:

1. Release the new version beside the old one and roll out as usual.
2. Before the vote, check that nearly all the stake runs a release with it (§8.3), in both the current committee and the next.
3. Governance sets `mpc_signing_version` (§6.6). Nodes without it refuse the new epoch. Too many of them stall the formation, holding deposits and withdrawals, until it is aborted at the end of its Sui epoch (§9.3). Upgrade them in time, or revert the key or set `reconfig_hold` before the abort; a hold only defers, so clear it once fixed.

Roll back only to a release that supports every pinned version. Before every release, stop if `hashi mpc show-reduction` differs between the running release and the candidate on any network.
