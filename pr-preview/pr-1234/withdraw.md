...
conditions. This minimum covers the worst-case miner fee (which pays for
both the user's transaction weight and opportunistic UTXO pool
consolidation) and the dust threshold for the user's output. See
...
Each committee member independently screens the Bitcoin destination address
and the requesting Sui address with its own TRM Labs account (see
[Handling Sanctioned Addresses](sanctions.mdx)). A member that rejects the
request does not vote to approve it. If a quorum still approves, all members
must assist in completing the withdrawal.
...
withdrawal record that all validators can independently verify. The
certificate also covers `change_spend`, the spend data stored with the change
UTXOs, and `sighash_digest`, a SHA-256 over every input's sighash. Each member
computes both itself, the sighashes from the inputs' stored spend data, and
signing later refuses a withdrawal whose sighashes do not hash to
`sighash_digest`.
