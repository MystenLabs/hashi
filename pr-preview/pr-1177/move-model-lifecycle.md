...
flowchart TD
    ...
    subgraph Committee["Hashi Protocol Committee"]
        ...
        SCREEN["AML screening<br/>(each member's TRM Labs account)"]
    ...
    MEMBERS -->|"Screen BTC tx<br/>& Sui recipient"| SCREEN
...
| 3    | Committee members observe, screen, and sign request | Each member screens with TRM Labs; BLS signatures aggregated → `CommitteeSignature`   |
...
flowchart TD
    ...
    subgraph Committee["Hashi Protocol Committee"]
        ...
        SCREEN2["AML screening<br/>(each member's TRM Labs account)"]
...
| 2    | Committee screens and approves request       | Each member screens with TRM Labs; `WithdrawalRequest` status → `Approved`   |
