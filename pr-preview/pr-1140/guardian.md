...
Before the ceremony, each KP provisions a YubiKey and supplies its public OpenPGP
certificate, primary-key fingerprint, and three matching PEM sidecars.
...
for device requirements, artifact filenames, and attestation policy and exclusions.
...
Signature verification, encryption, and recipient checks pin the actual attested
keys, not merely a primary-key fingerprint. This preserves one certificate and
one ciphertext per KP/share.
