...
sequenceDiagram
    ...
    KPs->>KPs: Sign(session_id, config_hash, encrypted share)
    KPs->>Proxy: SingleProvisionerInit(signed submission)
    Proxy->>Proxy: Pre-verify KP signature and relay roster
    Proxy->>Guardian: Fetch live session, config, and provisioning status
    Guardian-->>Proxy: session_id, config_hash, n/t, provisioned flag
    alt fewer than threshold shares collected
        ...
        Guardian->>Guardian: Verify signatures, session/config pins, and KP assignments
