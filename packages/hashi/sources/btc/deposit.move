...
public fun deposit(
    ...
    assert!(!hashi.borrow_bitcoin_state().utxo_pool().is_spent_or_active(request.utxo().id()));
...
public fun confirm_deposit(
    ...
    let request = hashi.borrow_bitcoin_state_mut().deposit_queue_mut().remove(request_id);
