...
/// own pass unconditionally; retired keys are rejected outright so they can
/// never reappear in a pinned config.
...
public(package) fun is_valid_value(key: &std::string::String, value: &config_value::Value): bool {
    ...
    } else if (k == &KEY_REMOVED_NONCE_GENERATION_PROTOCOL) {
        false
