...
impl GuardianS3Client {
    ...
    // ========================================================================
    // Plain (no Object-Lock) JSON object access
    // ========================================================================
    ...
    pub async fn write_at_key_no_lock<T: Serialize>(
        &self,
        key: &str,
        value: &T,
    ) -> GuardianResult<()> {
        let body = serde_json::to_string(value)
            .map_err(|e| S3Error(format!("failed to serialize value for key {key}: {e}")))?;
    ...
    /// Read and deserialize a JSON object at `key`.
    ///
    /// Read-side pair to [`Self::write_at_key_no_lock`]. Plain `get_object` +
    /// deserialize; lock-agnostic — works on object-lock-enabled or disabled
    /// buckets alike (unlike the private `get_object_unsafe`, which asserts lock
    /// metadata as part of the immutable-log trust model).
    pub async fn get_at_key<T: serde::de::DeserializeOwned>(&self, key: &str) -> GuardianResult<T> {
        let response = self
            .client
            .get_object()
            .bucket(self.config.bucket_name())
        ...
        serde_json::from_slice::<T>(&bytes).map_err(|e| {
            S3Error(format!(
                "Failed to deserialize object at key {}: {}",
                key, e
            ))
        })
    }
