...
impl GuardianReader {
    ...
    async fn read_latest_ceremony_log(
        ...
        let keys = self
            .s3
            .validate_prefix_history_and_list_keys(&CeremonyLogMessage::object_key_dir())
            .await?;
    ...
    async fn read_latest_committee_update(&mut self) -> GuardianResult<Option<Committee>> {
        let keys = self
            .s3
            .validate_prefix_history_and_list_keys(&CommitteeUpdateLogMessage::object_key_dir())
            .await?;
        let Some(key) = keys
            .into_iter()
    ...
    async fn read_genesis_log(&mut self) -> GuardianResult<Option<GenesisLogMessage>> {
        ...
        let keys = self
            .s3
            .validate_prefix_history_and_list_keys(&GenesisLogMessage::object_key_dir())
            .await?;
        if keys.is_empty() {
            return Ok(None);
        }
