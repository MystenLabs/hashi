...
pub async fn run(cfg: ProvisionerConfig) -> anyhow::Result<()> {
    ...
    maybe_submit_provisioner_init(
        cfg.guardian_endpoint.as_deref(),
        &session_id,
...
async fn maybe_submit_provisioner_init(
    endpoint: Option<&str>,
    ...
    precheck_guardian_endpoint(&mut client, expected_session_id, expected_guardian_config)
        .await
...
async fn precheck_guardian_endpoint(
