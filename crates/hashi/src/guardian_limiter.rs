...
//! Local emulator of the guardian's rate limiter.
//!
//! The guardian is the authoritative source of truth for the rate
//! bucket. `LocalLimiter` mirrors it so that:
//!
//! - coin selection can project capacity locally and stop batching when
//!   the bucket is exhausted, without a round-trip;
//! - the leader picks `next_seq` from local state rather than JIT-querying
//!   `GetGuardianInfo` per hard reserve; and
//! - every node (including non-leaders) stays close to the guardian's
//!   view via periodic reconciliation, so leader rotation is seamless.
//!
//! Consistency is enforced by splitting the write path into
//! `validate_consume` (pure check, returns the `seq` the caller must use)
//! and `apply_consume` (mutation, called only after the guardian has
//! accepted the request). On any unexpected guardian rejection the leader
//! calls [`LocalLimiter::snap_to`] to sync back to authoritative state.
...
/// Local emulator of the guardian rate limiter.
...
impl LocalLimiter {
    ...
    /// Read-only snapshot of the current state. Useful for metrics, for
    /// asserting parity with the guardian in tests, and for the coin
    /// selector's capacity cap.
    pub async fn snapshot(&self) -> LimiterState {
        *self.state.lock().await
    }

    /// Capacity projected to `timestamp_secs`, accounting for linear
    /// refill since `last_updated_at`. Clamped at the config ceiling.
    pub async fn capacity_at(&self, timestamp_secs: u64) -> u64 {
        let state = *self.state.lock().await;
        project_capacity(&self.config, &state, timestamp_secs)
    }

    /// The next `seq` the caller should send to the guardian. Snapshot
    /// read; the actual `validate_consume` re-checks under the mutex.
    pub async fn next_seq(&self) -> u64 {
        self.state.lock().await.next_seq
    }

    /// Check whether a consume for `(timestamp_secs, amount_sats)` would
    /// be valid against the current local state, and return the `seq`
    /// the caller should submit. Does **not** mutate state — the caller
    /// must call `apply_consume` after the guardian accepts, or
    /// `snap_to` / do nothing on rejection.
    ...
    /// Apply a consume that the guardian has already accepted. Mutates
    /// state the same way the guardian did.
    ///
    /// Errors (and leaves state untouched) when:
    /// - `applied_seq` is not equal to the current `next_seq` (drift);
    /// - `timestamp_secs < last_updated_at`; or
    /// - projected capacity is below `amount_sats`.
    ...
    /// Replace the local state with the guardian's authoritative snapshot.
    /// Called on startup (bootstrap), periodically (reconciliation), and
    /// whenever the guardian reports drift during a hard reserve.
