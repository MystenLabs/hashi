...
fn derived_threshold_rejects_a_faulty_bound_too_small_to_form_a_certificate() {
    ...
    assert!(
        matches!(err, MpcError::CryptoError(ref m) if m.contains("Invalid value")),
        "must be rejected by the reducer's input gate, not by the floor check or a \
         config parse error, got {err:?}"
    );
}
