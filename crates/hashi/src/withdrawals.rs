...
impl Hashi {
    ...
    /// Each validator independently reconstructs the `StandardWithdrawalRequest`
    /// from on-chain `WithdrawalTransaction` data and signs it with BLS. The
    /// leader aggregates these into a committee certificate for the guardian.
...
/// Build a guardian `StandardWithdrawalRequest` from an on-chain
/// `WithdrawalTransaction`.
///
/// Both the leader and each validator call this to deterministically
/// reconstruct the same request from on-chain state + the leader-provided
/// `timestamp_secs` and `seq`. The resulting request is then BLS-signed by the
/// committee so the guardian can verify its origin.
pub fn build_guardian_withdrawal_request(
    ...
    // Inputs: each selected UTXO becomes a guardian `InputUTXO`, keyed by the
    // taproot script-path artifacts derived from the per-input deposit pubkey.
    let inputs = txn
        .inputs
        .iter()
        .map(|utxo| {
    ...
    // Outputs: positional withdrawal outputs are `External`; the optional
    // change output (last, when there are more outputs than requests) is
    // `Internal` back to the hashi root (all-zero derivation path).
