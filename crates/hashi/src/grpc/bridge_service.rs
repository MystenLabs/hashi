...
use hashi_types::proto::SignCommitteeActivationResponse;
...
impl BridgeService for HttpService {
    ...
    #[tracing::instrument(
        level = "info",
        skip_all,
        fields(committee_epoch = tracing::field::Empty, caller = tracing::field::Empty),
    )]
    async fn sign_committee_activation(
        &self,
        request: Request<SignCommitteeActivationRequest>,
    ) -> Result<Response<SignCommitteeActivationResponse>, Status> {
        let caller = authenticate_caller(&request)?;
        tracing::Span::current().record("caller", tracing::field::display(&caller));
        let committee_epoch = request.get_ref().committee_epoch;
        tracing::Span::current().record("committee_epoch", committee_epoch);
        let member_signature = self
            .inner
            .validate_and_sign_committee_activation(committee_epoch)
            .map_err(|e| Status::failed_precondition(e.to_string()))?;
        tracing::info!(committee_epoch, "Signed committee activation");
        Ok(Response::new(SignCommitteeActivationResponse {
            member_signature: Some(member_signature),
        }))
    }
