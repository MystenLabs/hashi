...
pub struct Hashi {
    ...
    /// Lazy gRPC channel to the `hashi-guardian` rate limiter. `None` when
    /// `config.guardian_endpoint` is unset — that's the no-guardian baseline
    /// and is bit-for-bit equivalent to pre-integration hashi. Subsequent
