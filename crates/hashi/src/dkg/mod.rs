...
impl DkgManager {
    ...
    pub fn recover_from_storage_when_completed(
        ...
        for cert in &certificates {
            ...
            if mgr.dealer_outputs.contains_key(&dealer) {
                continue;
            }
            if !mgr.dealer_messages.contains_key(&dealer) {
                if let Ok(Some(msg)) = mgr.public_messages_store.get_dealer_message(&dealer) {
                    mgr.dealer_messages.insert(dealer, msg);
                } else {
                    tracing::info!(
                        "Cannot recover dealer {:?} - message not in local storage",
                        dealer
                    );
                    continue;
                }
            }
            if mgr.process_certified_dealer_message(dealer).is_err() {
                tracing::info!("Failed to process dealer {:?} message", dealer);
            }
        }
        let certified_dealers: Vec<_> = certificates
            .iter()
            .map(|c| c.message().dealer_address)
            .filter(|d| mgr.dealer_outputs.contains_key(d))
            .collect();
        let certified_weight: u64 = certified_dealers
            .iter()
            .filter_map(|d| mgr.committee.weight_of(d).ok())
            .sum();
        if certified_weight < threshold as u64 {
            return Err(DkgError::RecoveryFailed(format!(
                "insufficient local data (need weight {}, recovered {})",
                threshold, certified_weight
            )));
        }
    ...
    pub async fn run(
        ...
        if tob_channel.existing_certificate_weight() < threshold as u32
            && let Err(e) = Self::run_as_dealer(dkg_manager, p2p_channel, tob_channel).await
        {
            tracing::error!("Dealer phase failed: {}. Continuing as party only.", e);
        }
        Self::run_as_party(dkg_manager, p2p_channel, tob_channel).await
    }
    ...
    async fn run_as_dealer(
        ...
        let results = send_to_many(
            ...
            dealer_data.request,
            |addr, req| async move { p2p_channel.send_dkg_message(&addr, &req).await },
        )
        .await;
    ...
    async fn run_as_party(
        ...
        loop {
            ...
            }
            {
                let mut mgr = dkg_manager.lock().unwrap();
                if !mgr.dealer_outputs.contains_key(&dealer)
                    && !mgr.complaints_to_process.contains_key(&dealer)
                {
        ...
        let output = {
            let mgr = dkg_manager.lock().unwrap();
            mgr.complete_dkg(mgr.dealer_outputs.keys().cloned())?
        };
        Ok(output)
    }
    ...
    async fn retrieve_dealer_message(
        ...
        for signer in signers {
            if signer == self_address {
                continue;
            }
            match p2p_channel.retrieve_message(&signer, &request).await {
                Ok(response) => {
                    if compute_message_hash(&response.message) == message.message_hash {
                        let mut mgr = dkg_manager.lock().unwrap();
                        mgr.store_message(message.dealer_address, &response.message)?;
                        return Ok(());
                    }
                    tracing::info!(
    ...
    async fn recover_shares_via_complaint(
        ...
        for signer in signers {
            ...
            match receiver.recover(&message, responses.clone()) {
                Ok(partial_output) => {
                    let mut mgr = dkg_manager.lock().unwrap();
                    mgr.dealer_outputs.insert(*dealer, partial_output);
                    mgr.complaints_to_process.remove(dealer);
                    return Ok(());
                }
...
mod tests {
    ...
    async fn test_run_dkg() {
        ...
        let test_manager = Arc::new(Mutex::new(test_manager));

        // Call run_as_dealer() and run_as_party() for validator 0
        DkgManager::run_as_dealer(&test_manager, &mock_p2p, &mut mock_tob)
            .await
            .unwrap();
        let output = DkgManager::run_as_party(&test_manager, &mock_p2p, &mut mock_tob)
            .await
            .unwrap();
    ...
    fn setup_run_test() -> RunTestSetup {
        ...
        let test_manager = Arc::new(Mutex::new(managers.remove(0)));
    ...
    async fn test_run_triggers_dealer_phase() {
        ...
        let output = DkgManager::run(&setup.test_manager, &setup.mock_p2p, &mut mock_tob)
            .await
            .unwrap();
    ...
    async fn test_run_as_dealer_success() {
        ...
        let test_manager = Arc::new(Mutex::new(setup.create_manager(0)));
        ...
        // Call run_as_dealer()
        let result = DkgManager::run_as_dealer(&test_manager, &mock_p2p, &mut mock_tob).await;
    ...
    async fn test_run_as_party_recovers_shares_via_complaint() {
        ...
        let output = DkgManager::run_as_party(&party_manager, &mock_p2p, &mut mock_tob)
            .await
            .unwrap();
        ...
        assert!(
            !mgr.complaints_to_process.contains_key(&dealer_1_addr),
            "Complaint should be cleared after successful recovery"
        );
        assert!(
            mgr.dealer_outputs.contains_key(&dealer_1_addr),
    ...
    async fn test_run_as_party_requires_different_dealers() {
        ...
        let test_manager = Arc::new(Mutex::new(managers.remove(2)));
        let other_managers: HashMap<_, _> = managers
            .into_iter()
            .enumerate()
            .map(|(idx, mgr)| {
    ...
    async fn test_run_as_dealer_p2p_send_error() {
        ...
        let result = DkgManager::run_as_dealer(&test_manager, &failing_p2p, &mut mock_tob).await;
    ...
    async fn test_run_as_dealer_tob_publish_error() {
        ...
        let result =
            DkgManager::run_as_dealer(&test_manager, &succeeding_p2p, &mut failing_tob).await;
    ...
    async fn test_run_as_dealer_partial_failures_still_collects_enough() {
        ...
        let result =
            DkgManager::run_as_dealer(&test_manager, &partially_failing_p2p, &mut mock_tob).await;
    ...
    async fn test_run_as_party_tob_receive_error() {
        ...
        let result = DkgManager::run_as_party(&test_manager, &mock_p2p, &mut failing_tob).await;
    ...
    async fn test_run_as_party_retrieves_missing_dealer_messages() {
        ...
        assert!(mgr.dealer_outputs.contains_key(&dealer1_addr));
        assert!(mgr.dealer_outputs.contains_key(&dealer2_addr));
    }
    ...
    async fn test_run_as_party_aborts_on_failed_recovery() {
        ...
        let mgr = party_manager.lock().unwrap();
        assert!(
            !mgr.dealer_outputs.contains_key(&dealer1_addr),
        ...
        assert!(
            !mgr.dealer_outputs.contains_key(&dealer0_addr),
        ...
        assert!(
            mgr.complaints_to_process.contains_key(&dealer0_addr),
    ...
    async fn test_recover_shares_via_complaint_succeeds_with_exact_threshold() {
        ...
        let result = DkgManager::recover_shares_via_complaint(
            &party_manager,
            &dealer_addr,
            signer_addresses,
            &mock_p2p,
        ...
        let mgr = party_manager.lock().unwrap();
        assert!(mgr.dealer_outputs.contains_key(&dealer_addr));
        assert!(
            !mgr.complaints_to_process.contains_key(&dealer_addr),
            "Complaint should be cleared after successful recovery"
        );
    }
    ...
    async fn test_recover_shares_via_complaint_no_dealer_message() {
        ...
        let _ = DkgManager::recover_shares_via_complaint(
            &party_manager,
            &dealer_addr,
    ...
    async fn test_retrieve_dealer_message_success() {
        ...
        assert!(!mgr.dealer_outputs.contains_key(&dealer_address));
        drop(mgr);
    ...
    async fn test_retrieve_stores_invalid_message_for_later_complaint() {
        ...
        let receiver_manager = Arc::new(Mutex::new(receiver_manager));
        ...
        assert!(
            !mgr.dealer_outputs.contains_key(&dealer_addr),
    ...
    async fn test_restart_dealer_reuses_stored_message() {
        ...
        let result = DkgManager::run_as_dealer(&restarted_manager, &mock_p2p, &mut mock_tob).await;
        assert!(result.is_ok());
    ...
    async fn test_restart_party_uses_stored_messages_without_retrieval() {
        ...
        // Run as party
        let result = DkgManager::run_as_party(&party_manager, &tracking_p2p, &mut mock_tob).await;
        assert!(result.is_ok());
        ...
        assert!(
            mgr.dealer_outputs.contains_key(&dealer2_addr),
    ...
    #[test]
    fn test_recover_from_storage_when_completed_success() {
        let mut rng = rand::thread_rng();
        let setup = TestSetup::new(5);
        let committee = setup.committee_set.current_committee().unwrap().clone();

        // Create dealers and their messages
        let dealer_indices = [0usize, 1, 2];
        let mut dealer_messages = HashMap::new();

        for &idx in &dealer_indices {
            let mgr = setup.create_manager(idx);
            let msg = mgr.create_dealer_message(&mut rng);
            dealer_messages.insert(setup.address(idx), msg);
        }

        // Create certificates by having all parties sign each dealer's message
        let mut certificates = Vec::new();
        for &dealer_idx in &dealer_indices {
            let dealer_addr = setup.address(dealer_idx);
            let message = dealer_messages.get(&dealer_addr).unwrap();

            // Collect signatures from all validators
            let mut signatures = Vec::new();
            for validator_idx in 0..5 {
                let mut signer = setup.create_manager(validator_idx);
                let sig = receive_dealer_message(&mut signer, message, dealer_addr).unwrap();
                signatures.push(sig);
            }

            let cert =
                create_test_certificate(&committee, message, dealer_addr, signatures).unwrap();
            certificates.push(cert);
        }

        // Create a store pre-populated with the dealer messages (simulating restart)
        let store_count = Arc::new(AtomicUsize::new(0));
        let mut store = TrackingPublicMessagesStore::new(store_count.clone());
        for (addr, msg) in &dealer_messages {
            store.pre_populate(*addr, msg.clone());
        }

        // Create a fresh manager (simulating node restart)
        let receiver_idx = 3;
        let receiver_manager = setup.create_manager_with_store(receiver_idx, Box::new(store));
        let receiver_manager = Arc::new(Mutex::new(receiver_manager));

        // Recover from storage
        let result =
            DkgManager::recover_from_storage_when_completed(&receiver_manager, certificates);
        ...
        let output = result.unwrap();
        assert!(!output.key_shares.shares.is_empty());
        assert!(!output.commitments.is_empty());
    }

    #[test]
    fn test_recover_from_storage_when_completed_insufficient_data() {
        let mut rng = rand::thread_rng();
        let setup = TestSetup::new(5);
        let committee = setup.committee_set.current_committee().unwrap().clone();
        ...
        // Create certificates for all dealers
        let mut certificates = Vec::new();
        for &dealer_idx in &dealer_indices {
            let dealer_addr = setup.address(dealer_idx);
            let message = dealer_messages.get(&dealer_addr).unwrap();
        ...
        // Create store with only ONE dealer message (not enough to meet threshold)
        let store_count = Arc::new(AtomicUsize::new(0));
        let mut store = TrackingPublicMessagesStore::new(store_count.clone());
        let first_dealer = setup.address(dealer_indices[0]);
        store.pre_populate(
            first_dealer,
            dealer_messages.get(&first_dealer).unwrap().clone(),
        );
        ...
        // Recovery should fail due to insufficient local data
        let result =
            DkgManager::recover_from_storage_when_completed(&receiver_manager, certificates);
        ...
        assert!(
            ...
            "Error should be RecoveryFailed: {}",
            err
        );
        assert!(
            err.to_string().contains("insufficient local data"),
