...
pub fn sign_btc_tx(
    ...
    change_utxo: TxOut,
    keypair: &Keypair,
) -> GuardianResult<Vec<bitcoin::taproot::Signature>> {
    ...
    for (index, input_utxo) in input_utxos.iter().enumerate() {
        ...
        let sighash = sighasher
            ...
            .map_err(|e| GenericError(format!("sighash error {}", e)))?;
        let msg = Message::from_digest(*sighash.as_byte_array());

        let mut rng = thread_rng();
        let mut bytes = [0u8; 32];
        rand::Rng::fill(&mut rng, &mut bytes);
        let signature = secp.sign_schnorr_with_aux_rand(&msg, keypair, &bytes);
...
) -> (bitcoin::secp256k1::Keypair, bitcoin::Address) {
