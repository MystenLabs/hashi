...
pub struct TestNetworks {
    ...
    /// `Some` when `TestNetworksBuilder::with_guardian()` was used.
    /// Held for the lifetime of the test so the in-process gRPC server
    /// keeps serving; dropped alongside the rest of the networks.
...
impl TestNetworksBuilder {
    ...
    /// hashi node, and finalize it with the on-chain committee + BTC
    /// master pubkey once DKG completes. No-op when not set; the
    /// baseline test remains the no-guardian flow.
    ...
    pub async fn build(self) -> Result<TestNetworks> {
        ...
        // Start the guardian before hashi nodes when requested. The
        // harness's gRPC endpoint must exist by the time any hashi node
        // tries to reach it (`Hashi::start` lazy-connects, so nothing
        // fails immediately if the guardian is late, but having it up
        // first keeps the ordering honest for future pre-flight RPCs).
        ...
        // Finalize the guardian once the hashi committee + BTC master
        // pubkey are available on chain. `HashiNetworkBuilder::build`
        // already waits for the initial committee to form, but we
        // re-check the MPC key here because `get_onchain_mpc_pubkey`
        // reads from `CommitteeSet.mpc_public_key` which is populated
        // atomically with the `end_reconfig` event.
...
/// Finalize the in-process guardian with the committee + BTC master
/// pubkey that just came out of hashi's DKG. Called from
/// `TestNetworksBuilder::build` when `with_guardian` is set.
async fn finalize_guardian_harness(networks: &mut TestNetworks) -> Result<()> {
    ...
    // Any node's on-chain state is fine — they converge after DKG.
