...
pub struct HashiNetworkBuilder {
    ...
    /// If set, every hashi node's `guardian_endpoint` is populated with
    /// this value. Typically points at an in-process `GuardianHarness`
    /// started earlier by `TestNetworksBuilder`.
